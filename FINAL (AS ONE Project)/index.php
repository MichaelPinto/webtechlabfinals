...
if(isset($_POST['Submit']))
{
    ...
    $result = mysqli_query($register->$DB_con, $query);
    $resultcheck = mysqli_num_rows($result);

    if ($resultcheck > 0){
        $register->url("../registration.php?signup=usertaken");
        exit();
    }

    $stmt = $DB_con->prepare('INSERT INTO users(username, user_type, firstname, lastname, gender, birthdate, middle_initial, 
    password, email, nickname, address, contact_no) VALUES (:username, :usertype, :firstname, :lastname, :gen, :birthdate, :middlename, :password, :email, 
    :nickname, :address, :contact)');
